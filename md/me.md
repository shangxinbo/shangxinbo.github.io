...
1. [北京海迈德科技咨询有限公司](http://www.high-med.com)
   ...
   > 初级前端开发工程师
   >
   > 毕业后的首就业，干的活比较杂，原型设计，UI作图切图，前端，服务端，原生应用，微信程序，网站服务全栈技能样样都做，相信技术人都懂的。感谢我那想干嘛就干嘛的领导们，让我练就一身好技能。
...
2. [北京拉手网络技术有限公司](http://www.lashou.com)
   ...
   * [拉手团购主站](http://www.lashou.com)
     ...
     这是我在拉手网的主要工作，现版本的'首页','身边团购','今日新单'，'美食'，‘电影’，‘酒店’，‘旅游’，‘订单’，‘支付’等主要模块都是由我改版。使用了百度的[FIS](http://fis.baidu.com/)前端解决方案。体会到前端其实是在解决表现层的东西，表现层是指用户能够看到的东西，可交互的东西，那这样理解就不再是单纯的从编程语言来区分工种而从技术架构上区分。client端的东西都可以理解成前端，包括手机app，webapp，电视app，手表app……

   * [修好了](http://www.xiuhaole.com)

     `FIS` `bootstrap` 

     这个项目是4端同时进行，管理后台，PC端，mobile端，工程师端。这样做才真正体会出前后端分离的好处，server API 和client可以是一对多的关系。这是拉手网被收购后唯一一次可以拿得出手的尝试。主前端是我，另外有两个前端。
...
3. [北京集奥聚合科技有限公司](http://www.geotmt.com)
   ...
   * [方舟](http://ark.geotmt.com)
     ...
     数据营销平台，前端对接java&php双端，所以，前端code要完全独立于后端code，双方通过restfull api 对接。前端独立部署。脱离了FIS后使用npm包管理，使用gulp做自动化处理。使用前端模板引擎artTemplate,整体的项目结构和方案，我剥离出一个项目初始化库 [sea-init](https://github.com/shangxinbo/seajs-init) 。

   * 大数据可视化

     `sea.js` `HTML5 & CSS3`  `d3.js` `echarts.js` `express` `request` `mongodb` `gulp` `vue` `node` `4k`

     将公司大数据分析的人群结果用图表及图形展示在大屏幕上，大屏幕是一个16块显示器组成的4k屏。项目是用node层做一层缓存以防宕机，同时使用node绕过session校验（后端的实现是一坨屎）。前期使用sea-init做的项目，后使用vue做的第二个版本。图标使用echarts和d3结合，部分动画是CSS3动画所做（不是我做的动画）
